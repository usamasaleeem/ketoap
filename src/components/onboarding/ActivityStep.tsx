...
const activityLevels = [
  {
    ...
    icon: <Sofa className="size-5 sm:size-6" />,
  ...
  },
  {
    ...
    icon: <Footprints className="size-5 sm:size-6" />,
  ...
  },
  {
    ...
    icon: <Bike className="size-5 sm:size-6" />,
...
export function ActivityStep({ data, onChange }: StepProps) {
  ...
  return (
    <div className="space-y-6">
        ...
        {activityLevels.map((level, index) => {
          ...
          return (
            <motion.button
              ...
              className={`relative w-full group transition-all duration-300 ${selected ? 'scale-[1.01] sm:scale-[1.02]' : 'hover:scale-[1.01]'
                }`}
            >
              <div
                className={`relative w-full flex items-start gap-3 sm:gap-4 p-4 sm:p-5 rounded-2xl border-2 text-left transition-all duration-300 ${selected
              ...
              >
                ...
                <div
                  className={`relative flex items-center justify-center size-12 sm:size-14 rounded-2xl transition-all duration-300 flex-shrink-0 ${selected
                ...
                {/* Content - with better text sizing */}
                  ...
                  <div className="flex items-center flex-wrap gap-1.5 sm:gap-2 mb-0.5 sm:mb-1">
                    ...
                    {selected && (
                      <motion.span
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        className="text-[10px] sm:text-xs font-medium text-primary bg-primary/10 px-1.5 sm:px-2 py-0.5 rounded-full"
                      >
                        Selected
                      </motion.span>
                    )}
                  </div>
                  <p className={`text-xs sm:text-sm transition-colors duration-300 ${selected ? 'text-foreground' : 'text-muted-foreground'
                    }`}>
                    {selected ? level.detailedDesc : level.description}
                  </p>
                </div>

                {/* Calorie indicator - hidden on small screens, shown on larger */}
                <div className={`hidden sm:block flex-shrink-0 self-center text-center transition-all duration-300 ${selected ? 'opacity-100' : 'opacity-40 group-hover:opacity-70'
                  }`}>
                  <div className="text-[10px] font-medium text-muted-foreground">Est. Daily</div>
                ...
                <div className={`hidden sm:flex flex-shrink-0 self-center transition-all duration-300 ${selected
      ...
      {/* Selected activity summary - responsive */}
      <AnimatePresence>
        {selectedLevel && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="p-3 sm:p-4 rounded-xl bg-gradient-to-br from-primary/5 via-primary/5 to-transparent border border-primary/10"
          >
            <div className="flex flex-wrap items-center gap-3 sm:gap-4">
              <div className={`p-2 sm:p-3 rounded-xl bg-gradient-to-br ${selectedLevel.color}`}>
              ...
              <div className="flex-1 min-w-[120px]">
                <div className="flex items-center flex-wrap gap-1.5 sm:gap-2">
                  <p className="text-sm sm:text-base font-medium">
                    Activity: <span className="text-primary">{selectedLevel.label}</span>
                  </p>
              ...
              <div className="text-right ml-auto">
                <div className="text-[10px] sm:text-xs text-muted-foreground">Daily Estimate</div>
                <div className="text-base sm:text-lg font-bold text-primary">{selectedLevel.calories}</div>
                <div className="text-[10px] text-muted-foreground">calories</div>
              </div>
            </div>
          </motion.div>
